...
static void TestImuNavigation(void)
{
  ...
  snapshot.angle_frames++; snapshot.angle_tick = 200U; snapshot.angle[2] = 5461; /* +30 deg */
  assert(ImuNavigation_Update(&nav, &snapshot, 200U, 0.25f));
  snapshot.angle_frames++; snapshot.angle_tick = 300U; snapshot.angle[2] = 10923; /* +60 deg total */
  assert(ImuNavigation_Update(&nav, &snapshot, 300U, 0.77f));
  assert(nav.heading_rad > 1.29f && nav.heading_rad < 1.31f);
  ...
  assert(ImuNavigation_Update(&nav, &snapshot, 1000U, -0.5f));
  ...
  snapshot.angle_frames++; snapshot.angle_tick = 1010U; snapshot.angle[2] = 10923;
  assert(ImuNavigation_Update(&nav, &snapshot, 1010U, 1.5f));
  assert(nav.rejected_steps == 1U);
  assert(nav.heading_rad < -0.79f && nav.heading_rad > -0.80f);
  assert(nav.reference_yaw_raw == 0); /* A >45-degree frame cannot re-anchor. */
}
...
static void TestCombatStrategy(void)
{
  ...
  assert(CombatStrategy_ShouldReturn(6000U) == 0U);
  ...
  assert(CombatStrategy_ShouldReturn(241001U) == 1U);
  selection = CombatStrategy_SelectTarget(&pose, candidates, 2U, 242000U);
  assert(selection.valid == 0U); /* other target is too far in endgame */
  CombatStrategy_GetPatrolWaypoint(&patrol_x, &patrol_y, 242000U);

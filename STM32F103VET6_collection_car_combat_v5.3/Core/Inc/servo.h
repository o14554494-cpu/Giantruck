/**
  ...
  *     受上底板限制不能 360° 旋转，路径为：
  *     初始垂直(90°) -> 逆时针转90°(180°) -> 顺时针转180°(0°)清理剩余物块
  *     -> 恢复垂直(90°)，卸货期间循环。
...
/* 普通舵机舱门扫摆路径参数 ------------------------------------------------
 * 路径:初始垂直(90°) -> 逆时针转90°(180°) -> 顺时针转180°(0°)清理剩余物块
 *      -> 恢复垂直(90°)，循环往复。
 * 若装好后实际转向相反，把 CCW_DEG 和 CW_DEG 两个值对调即可。 */
#define SERVO_HATCH_INITIAL_DEG  90U    /* 初始位置:垂直 */
#define SERVO_HATCH_CCW_DEG      180U   /* 逆时针转90°后到达的角度 */
#define SERVO_HATCH_CW_DEG       0U     /* 顺时针转180°后到达的角度(清理) */
#define SERVO_HATCH_MOVE90_MS    500U   /* 每段 90° 行程用时 */
#define SERVO_HATCH_SWEEP180_MS  1000U  /* 180° 清理行程用时 */
#define SERVO_HATCH_CYCLE_MS                                             \
  (SERVO_HATCH_MOVE90_MS + SERVO_HATCH_SWEEP180_MS + SERVO_HATCH_MOVE90_MS)
...
/* 进入卸货(EJECT)阶段时调用：启动持续运动。重复调用不会重置扫摆相位。 */

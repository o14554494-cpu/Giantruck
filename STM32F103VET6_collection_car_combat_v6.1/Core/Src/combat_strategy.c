...
static float progress_x_mm;
static float progress_y_mm;
static uint32_t progress_tick;
...
uint8_t CombatStrategy_ShouldReturn(uint32_t now_ms)
{
  ...
  if ((combat_active == 0U) || (payload_count == 0U)) return 0U;
  elapsed = CombatStrategy_ElapsedMs(now_ms);
  batch_size = elapsed >= 210000U ? COMBAT_BATCH_SIZE_LATE :
                                   COMBAT_BATCH_SIZE_NORMAL;

  return (elapsed >= COMBAT_ENDGAME_RETURN_MS) ||
         (payload_count >= batch_size) ||
         ((now_ms - last_deposit_tick) >= COMBAT_DEPOSIT_INTERVAL_MS) ||
         (opponent_exit_required != 0U);
}
                                   ...
                                   uint32_t now_ms)
{
  float dx;
  float dy;
  if ((combat_active == 0U) || (pose == NULL)) return 0U;

  if ((progress_tick == 0U) || (forward_commanded == 0U))
  {
    progress_x_mm = pose->x_mm;
    progress_y_mm = pose->y_mm;
    progress_tick = now_ms;
    return 0U;
  }

  dx = pose->x_mm - progress_x_mm;
  dy = pose->y_mm - progress_y_mm;
...
const char *CombatStrategy_PhaseName(uint32_t now_ms)
{
  ...
  if (payload_count >= COMBAT_BATCH_SIZE_NORMAL) return "RETURN";

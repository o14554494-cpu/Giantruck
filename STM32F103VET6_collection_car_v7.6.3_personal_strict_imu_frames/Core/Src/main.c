...
#define COMPETITION_GREEDY_ONLY       1U
#endif
...
#define VISION_WALL_STOP_MS                 150U
#define VISION_WALL_BACKUP_MS               400U
#define VISION_WALL_TURN_MIN_MS             650U
#define VISION_WALL_TURN_MAX_MS            2200U
#define VISION_WALL_BACKUP_PERCENT           20
#define VISION_WALL_TURN_PERCENT             30
...
#define GREEDY_IMU_LOCAL_PERIOD_MS           20000U
...
static WallAvoidPhase_t wall_avoid_phase;
static uint32_t wall_avoid_phase_tick;
static int8_t wall_avoid_turn_sign;
...
static void GreedyImuZero_Start(uint32_t now, uint8_t to_unload);
...
static void Competition_StartGreedy(void);
...
static void WallAvoid_Start(uint32_t now);
static void WallAvoid_Update(uint32_t now);
...
static const char *Auto_StateName(AutoState_t state)
{
  switch (state)
  {
    ...
    case AUTO_WALL_AVOID:  return "WALL_AVOID";
...
static void Motor_ProcessCommand(uint8_t command)
{
  ...
  /* The competition image is autonomous after power-up.  Keep only status
   * and explicit stop/hold commands reachable over Bluetooth so an accidental
   * phone keystroke cannot switch to another mode or start a bench action. */
  if (command != 'S' && command != 'X' && command != '0' &&
      command != 'H' && command != 'V' && command != '\r' && command != '\n')
  {
    Motor_SendText("COMPETITION GREEDY ONLY: CMD IGNORED; S=STOP V=STATUS\r\n");
    return;
  }
#endif
  ...
  switch (command)
  {
    ...
    case 'D':
    {
      ...
      Auto_Start();
      greedy_active = 1U;
      /* D captures the same stationary startup reference as Q before it
       * enters the unload sequence. */
      ...
      GreedyImuZero_Start(now, 1U);
      Motor_SendText("MODE=D_BLACK_UNLOAD: KEEP STILL FOR IMU ZERO 10 FRAMES, THEN WALL RETURN\r\n");
      break;
    }
    ...
    case 'Q':
    {
      ...
      GreedyImuZero_Start(now, 0U);
      Motor_SendText("MODE=GREEDY: KEEP STILL FOR IMU ZERO 10 FRAMES, THEN PICK/SCAN/BLACK UNLOAD\r\n");
      Motor_SendText("IMU ZERO=PRESERVED START YAW; NO ENCODER RE-ANCHOR AFTER GAP/JUMP\r\n");
      Motor_SendText(BrushFeedback_Cpr() >= 4U ?
          "AUTO_UNJAM ARMED: WATCH=RPM REVERSE=ENCODER_2_TURNS\r\n" :
          "AUTO_UNJAM ARMED: WATCH=PULSE REVERSE=TIMED; CPR UNKNOWN\r\n");
      break;
    }
...
static void Motor_SendStatus(void)
{
  ...
  CollectorFeedback_SendStatus();
  (void)snprintf(status, sizeof(status),
                 "UNLOAD=%s PAYLOAD=%u SERVO=%s REAR=%lu mm BRUSH=CONT PWM=%u/1000\r\n",
  ...
  Motor_SendText(status);
  (void)snprintf(status, sizeof(status),
                 "MODE=%s DBG=%s PICK=%d BTDROP=%lu VWALL=%s WRAW=%u WAGE=%lu\r\n",
  ...
  Motor_SendText(status);
  (void)snprintf(status, sizeof(status),
                 "IMU_NAV=%s HEAD=%ld REF=%u REF_RAW=%d ACC_RAW=%d OBS=%lu REJECT=%lu REC=%u/%u STALE=%lu OKREC=%lu RST=%lu SIGN=%d\r\n",
                 ...
                 (int)imu_navigation.last_yaw_raw,
                 (unsigned long)imu_navigation.observed_angle_frames,
                 (unsigned long)imu_navigation.rejected_steps,
                 (unsigned)imu_navigation.recovery_samples,
                 (unsigned)IMU_NAV_RECOVERY_REQUIRED_FRAMES,
                 (unsigned long)imu_navigation.stale_recoveries,
                 (unsigned long)imu_navigation.recovery_accepts,
                 (unsigned long)imu_navigation.recovery_resets,
                 IMU_NAV_YAW_SIGN > 0.0f ? 1 : -1);
  Motor_SendText(status);
  (void)snprintf(status, sizeof(status),
                 "IMU_RX ANGLE=%lu AGE=%ld CHECKERR=%lu TYPEERR=%lu GAPRESET=%lu UARTERR=%lu REARMERR=%lu\r\n",
                 (unsigned long)imu_snapshot.angle_frames,
                 imu_snapshot.angle_frames != 0U ?
                   (long)(uint32_t)(now - imu_snapshot.angle_tick) : -1L,
                 (unsigned long)imu_snapshot.checksum_errors,
                 (unsigned long)imu_snapshot.type_errors,
                 (unsigned long)imu_snapshot.stream_timeouts,
                 (unsigned long)imu_uart_errors,
...
static void WallAvoid_Start(uint32_t now)
{
  float dx = 1500.0f - robot_pose.x_mm;
  float dy = 1000.0f - robot_pose.y_mm;
  float centre_on_left = -Planner_Sin(robot_pose.heading_rad) * dx +
                          Planner_Cos(robot_pose.heading_rad) * dy;

  Motor_Stop();
  current_target_id = 0U;
  GreedyLocal_Reset(now, 0U);
  wall_avoid_turn_sign = centre_on_left >= 0.0f ? 1 : -1;
  wall_avoid_phase = WALL_AVOID_STOP;
  wall_avoid_phase_tick = now;
  auto_state = AUTO_WALL_AVOID;
  auto_state_start_tick = now;
  greedy_action = "WALL_STOP";
  Motor_SendText("VISION WALL CONFIRMED: STOP, BACKUP, TURN TO ARENA CENTRE\r\n");
}

static void WallAvoid_Update(uint32_t now)
{
  uint32_t elapsed = (uint32_t)(now - wall_avoid_phase_tick);

  if (wall_avoid_phase == WALL_AVOID_STOP)
  {
    Motor_Stop();
    if (elapsed >= VISION_WALL_STOP_MS)
    {
      wall_avoid_phase = WALL_AVOID_BACKUP;
      wall_avoid_phase_tick = now;
      greedy_action = "WALL_BACK";
    }
    return;
  }
  if (wall_avoid_phase == WALL_AVOID_BACKUP)
  {
    Motor_SetTarget(-VISION_WALL_BACKUP_PERCENT,
                    -VISION_WALL_BACKUP_PERCENT);
    if (elapsed >= VISION_WALL_BACKUP_MS)
    {
      Motor_Stop();
      wall_avoid_phase = WALL_AVOID_TURN;
      wall_avoid_phase_tick = now;
      greedy_action = "WALL_TURN";
    }
    return;
  }

  Motor_SetTarget((int16_t)(-wall_avoid_turn_sign * VISION_WALL_TURN_PERCENT),
                  (int16_t)( wall_avoid_turn_sign * VISION_WALL_TURN_PERCENT));
  if ((elapsed >= VISION_WALL_TURN_MIN_MS &&
       (Vision_WallFresh(now) == 0U || vision_wall_confirmed == 0U)) ||
      elapsed >= VISION_WALL_TURN_MAX_MS)
  {
    Motor_Stop();
    if (elapsed >= VISION_WALL_TURN_MAX_MS && vision_wall_confirmed != 0U)
      Motor_SendText("VISION WALL STILL PRESENT AFTER TURN: RESCAN ONLY\r\n");
    if (greedy_active != 0U)
...
static void Greedy_UpdateScan(uint32_t now)
{
  ...
  /* Refresh is due every 20 seconds, but execute it only at this already
   * stopped scan boundary. Never interrupt a turn, approach or blind pickup. */
  if (greedy_scan_turning == 0U && greedy_scan_steps == 0U &&
      greedy_imu_local_reference_valid != 0U &&
      (uint32_t)(now - greedy_imu_local_last_refresh_tick) >=
          GREEDY_IMU_LOCAL_PERIOD_MS)
  {
    Motor_SendText("IMU PERIODIC REFRESH DUE: AGE>=20S; STOP 300MS + 6 STABLE FRAMES\r\n");
  ...
  if (greedy_scan_steps >= GREEDY_SCAN_STEPS)
  {
    ...
    Motor_SendText("GREEDY EMPTY OBJECT SCAN: STOP/IMU QUIET 5 FRAMES, THEN ALIGN SAVED START 0 DEG\r\n");
    return;
  }
...
static void Auto_Start(void)
{
  ...
  BlackBump_Reset(1U);
  wall_avoid_phase = WALL_AVOID_STOP;
  wall_avoid_phase_tick = 0U;
  wall_avoid_turn_sign = 1;
  current_target_id = 0U;
  plan_dirty = 0U;
...
static void IMU_SendStatus(uint32_t now)
{
  ...
  (void)snprintf(line, sizeof(line),
      "IMU %s BAUD=%lu RX=%lu FRAME=%lu ANGLE=%lu AGE=%ld CHECKERR=%lu TYPEERR=%lu GAPRESET=%lu UARTERR=%lu REARMERR=%lu\r\n",
      ...
      (unsigned long)s.checksum_errors, (unsigned long)s.type_errors,
      (unsigned long)s.stream_timeouts, (unsigned long)imu_uart_errors,
      (unsigned long)imu_rearm_errors);
  Motor_SendText(line);
  if (s.angle_frames)
  {
...
static void GreedyImuZero_Start(uint32_t now, uint8_t to_unload)
{
  ...
  greedy_imu_zero_to_unload = to_unload;
  greedy_imu_zero_samples = 0U;
...
static void GreedyImuZero_Update(uint32_t now)
{
  ...
  if (greedy_imu_zero_to_unload != 0U)
  {
    auto_state = AUTO_UNLOAD_ALIGN_ZERO;
    auto_state_start_tick = now;
    BlackAlignStable_Reset();
...
static void Competition_StartGreedy(void)
{
  ...
  /* The competition image starts the same Q pipeline automatically.  Keep
   * the startup sequence stopped while the IMU collects ten stable frames;
   * after that gate, GreedyImuZero_Update starts the normal six-step scan. */
  servo_test_active = 0U;
  Servo_Stop();
  CombatStrategy_Stop();
  Auto_Start();
  ...
  collector_healthy_tracking = 0U;
  GreedyImuZero_Start(now, 0U);
  Motor_SendText("COMPETITION GREEDY AUTO: STARTED ON POWER-UP; KEEP STILL FOR IMU ZERO 10 FRAMES\r\n");
  Motor_SendText("AUTO FLOW: SCAN/PICK -> BLACK RETURN -> HATCH UNLOAD; BLUETOOTH START COMMANDS DISABLED\r\n");
}
...
static void BlackUnload_Update(uint32_t now)
{
  ...
  if (auto_state == AUTO_UNLOAD_SEARCH_FORWARD)
  {
    ...
    if (vision_wall_confirmed != 0U && Vision_WallFresh(now) != 0U)
    {
      Motor_Stop();
      Auto_Stop(0U);
      Motor_SendText("FORWARD BLACK SEARCH STOPPED: BLUE WALL AHEAD\r\n");
      return;
    }
    if (straight_hold_active == 0U ||
        straight_hold_base_percent != BLACK_RETURN_FORWARD_PERCENT ||
...
static void Auto_Update(void)
{
  ...
  if (auto_state == AUTO_WALL_AVOID)
  {
    WallAvoid_Update(now);
    return;
  }

  /* $W is independent of the target list. Two fresh W=1 packets are needed
   * before interrupting autonomous travel, rejecting one noisy blue frame. */
  if (Vision_WallFresh(now) != 0U && vision_wall_confirmed != 0U &&
      (auto_state == AUTO_NAVIGATE || auto_state == AUTO_FINAL_ALIGN ||
       auto_state == AUTO_COLLECT ||
       auto_state == AUTO_COMBAT_PATROL))
  {
    WallAvoid_Start(now);
...
static void CollectorFeedback_Update(void)
{
  ...
  if ((event == BRUSH_EVENT_LOW_RPM || event == BRUSH_EVENT_NO_PULSES ||
      ...
      collector_auto_attempts < BRUSH_AUTO_MAX_ATTEMPTS)
  {
    CollectorRecovery_Start();
    if (collector_recovery_state == COLLECTOR_STOPPING)
    {
      collector_auto_attempts++;
...
int main(void)
{
  ...
  Buzzer_Init();
  Vision_SendMode('S', 0U);
  Motor_SendText("READY FW=v7.6.3+PERSONAL_STRICT_IMU_FRAMES_UNLOAD30\r\n");
  Motor_SendText("AUTO START=POWER_UP; IMU LOCAL REFRESH=20S SAFE SCAN BOUNDARY; V=STATUS\r\n");

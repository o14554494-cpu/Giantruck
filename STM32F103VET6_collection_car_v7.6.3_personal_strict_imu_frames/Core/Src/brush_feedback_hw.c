...
{
  /* A/B must share GPIOB: one volatile IDR read, not two sequential samples. */
  uint32_t pins = BRUSH_ENCODER_A_PORT->IDR;
  return (uint8_t)((pins & BRUSH_ENCODER_A_PIN ? 2U : 0U) |
                  (pins & BRUSH_ENCODER_B_PIN ? 1U : 0U));
}
...
void BrushFeedback_HardwareInit(void)
{
  ...
  if (BRUSH_ENCODER_A_PORT != BRUSH_ENCODER_B_PORT) { Error_Handler(); return; }

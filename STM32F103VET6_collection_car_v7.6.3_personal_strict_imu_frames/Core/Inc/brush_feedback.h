...
typedef struct
{
  ...
  uint32_t errors;
  uint32_t error_delta, error_window, error_streak;
  uint8_t encoder_fault; /* Current window/streak fault, distinct from last_event. */
  uint32_t counts_per_rev;
  int32_t rpm_x10;
  int32_t delta_counts; /* Signed encoder edges in the latest sample. */
  uint8_t forward_motion; /* RPM threshold when calibrated, positive pulses otherwise. */
  uint16_t adc_raw;
  uint16_t adc_mv;
...
/* Q: arm without CPR calibration; require fresh forward motion before auto reversal. */

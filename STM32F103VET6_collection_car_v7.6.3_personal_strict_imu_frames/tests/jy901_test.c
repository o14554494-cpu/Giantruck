...
int main(void)
{
  ...
  /* A checksum-valid but unsupported frame type is not accepted. */
  packet(0x5A,4,5,6); feed(1800); JY901_Snapshot(&s);
  assert(s.type_errors==1 && s.frames==5 && s.angle_frames==2);
  ...
  /* A long gap in the middle of a packet discards the partial packet before
   * accepting the next complete header. */
  packet(0x53,7,8,9000);
  for(i=0;i<4;i++) JY901_RxByte(p[i],2100);
  feed(2100 + JY901_INTERBYTE_TIMEOUT_MS + 1U); JY901_Snapshot(&s);
  assert(s.angle[2]==9000 && s.stream_timeouts==1);
  puts("PASS: JY901 signed data, checksum/type filtering, timeout resync, freshness and yaw wrap");
  return 0;
}

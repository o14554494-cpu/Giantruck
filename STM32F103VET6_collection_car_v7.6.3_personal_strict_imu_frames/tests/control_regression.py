...
names = ['IMU_CopySnapshot', 'IMU_FormatAngle', 'IMU_StartRelativeReference',
         ...
         'Vision_ProcessIncoming', 'Vision_WallFresh', 'WallAvoid_Start',
         'WallAvoid_Update',
...
static void imu_nav_move_to(int16_t target_yaw)
{
  JY901Snapshot snapshot;
  int16_t current_yaw;
  int32_t remaining;
  int32_t step;
  JY901_Snapshot(&snapshot);
  current_yaw = snapshot.angle[2];
  do
  {
    remaining = JY901_RelativeYawRaw(target_yaw, current_yaw);
    step = remaining;
    if (step > 1000) step = 1000;
    if (step < -1000) step = -1000;
    current_yaw = (int16_t)((int32_t)current_yaw + step);
    clock_ms += 20U;
    imu_angle(0, 0, current_yaw);
    IMU_NavigationUpdate(clock_ms);
  } while (remaining != step);
}
static void imu_nav_wait(int16_t yaw, uint32_t duration_ms)
{
  uint32_t step_ms;
  do
  {
    step_ms = duration_ms > 100U ? 100U : duration_ms;
    clock_ms += step_ms;
    imu_angle(0, 0, yaw);
    IMU_NavigationUpdate(clock_ms);
    duration_ms -= step_ms;
...
static void test_greedy_relative_imu_telemetry(void)
{
  ...
  imu_angle(0, 0, 4000); IMU_NavigationUpdate(clock_ms);
...
static void test_imu_pid_turn_commands(void)
{
  ...
  imu_navigation.last_tick = clock_ms;
  assert(BlackUnload_Align(0.0f, clock_ms));
  puts("PASS: L/R stay manual; continuous IMU PID alignment corrects overshoot and settles");
}
...
static void test_feedback_commands_and_auto(void)
{
  ...
  finish_encoder_cycle();
  for (attempt = 3; attempt <= BRUSH_AUTO_MAX_ATTEMPTS; attempt++)
  {
    await_auto_stall();
...
static void test_q_auto_unjam(void)
{
  ...
  assert(BrushFeedback_Cpr() == 0);
  assert(strstr(test_log, "WATCH=PULSE REVERSE=TIMED"));
  for (i = 0; i < 11; i++)
  {
    feedback_steps(4); clock_ms += 100; CollectorFeedback_Update();
  }
  ...
  clock_ms += COLLECTOR_DIRECTION_PAUSE_MS; CollectorRecovery_Update();
  finish_recovery();
  for (i = 3; i <= BRUSH_AUTO_MAX_ATTEMPTS; i++)
  {
    await_auto_stall();
  ...
  await_auto_stall();
  assert(collector_recovery_state == COLLECTOR_HOLD);
  assert(!BrushFeedback_Snapshot(clock_ms).enabled); assert_recovery_stopped();
  Motor_ProcessCommand('Q'); /* A fault cannot be silently bypassed. */
  assert(collector_recovery_state == COLLECTOR_HOLD);
  ...
  assert(BrushFeedback_Snapshot(clock_ms).enabled);
  assert(strstr(test_log, "WATCH=RPM REVERSE=ENCODER_2_TURNS"));
  feedback_steps(4); clock_ms += 100; CollectorFeedback_Update();
  await_auto_stall(); assert(collector_use_encoder); finish_encoder_cycle();
  assert(greedy_active && auto_state == AUTO_SCAN);
  puts("PASS: power-up Q alone arms pulse/calibrated monitoring; auto reverse, backup stop, Q/count resume, retry/HOLD, missing feedback and explicit disarm");
}
...
static void test_greedy_step_scan(void)
{
  ...
  assert(strstr(test_log, "ALIGN SAVED START 0 DEG"));
  ...
  imu_nav_wait(5000, BLACK_HEADING_SETTLE_MS + AUTO_UPDATE_PERIOD_MS);
  fresh(); Auto_Update();
  ...
  imu_nav_move_to(13192);
  imu_nav_wait(13192, BLACK_HEADING_SETTLE_MS + AUTO_UPDATE_PERIOD_MS);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90);
  imu_nav_move_to(19564);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90);
  assert(left_target_percent < 0 && right_target_percent > 0);
  imu_nav_move_to(21384);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90 && !motor_running);
  imu_nav_wait(21384, BLACK_HEADING_SETTLE_MS + AUTO_UPDATE_PERIOD_MS);
  fresh(); Auto_Update();
  ...
  imu_nav_move_to(24576);
  imu_nav_wait(24576, BLACK_HEADING_SETTLE_MS + AUTO_UPDATE_PERIOD_MS);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90);
  imu_nav_move_to(29564);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90);
  assert(left_target_percent < 0 && right_target_percent > 0);
  imu_nav_move_to(32700);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90);
  imu_nav_move_to(-27768);
  fresh(); Auto_Update();
  assert(auto_state == AUTO_UNLOAD_TURN_CCW_90 && !motor_running);
  imu_nav_wait(-27768, BLACK_HEADING_SETTLE_MS + AUTO_UPDATE_PERIOD_MS);
  fresh(); Auto_Update();
  ...
  for (i = 0; i < BLACK_VISION_CONFIRM_PACKETS; i++)
  {
    ...
    Vision_ProcessIncoming();
    imu_angle(0, 0, -27768); IMU_NavigationUpdate(clock_ms);
    fresh(); Auto_Update();
  }
...
static void test_greedy_periodic_imu_refresh(void)
{
  reset_test(); Motor_ProcessCommand('Q'); lock_task_imu_zero();
  assert(imu_navigation.reference_yaw_raw == 5000);
  test_log[0] = '\0';
  clock_ms = greedy_imu_local_last_refresh_tick + GREEDY_IMU_LOCAL_PERIOD_MS;
  fresh();
  Auto_Update();
  assert(greedy_imu_local_pending && !motor_running);
  assert(strstr(test_log, "IMU PERIODIC REFRESH DUE: AGE>=20S"));
  lock_local_imu_reference(5100);
  assert(imu_navigation.reference_yaw_raw == 5000); /* startup zero immutable */
  assert(imu_local_navigation.reference_yaw_raw == 5100);
  assert(auto_state == AUTO_SCAN && !motor_running);
  puts("PASS: 20s deadline triggers six-frame calibration only at a stopped scan boundary and preserves startup zero");
}
...
static void test_openmv_wall_avoidance(void)
{
  reset_test();
  Motor_ProcessCommand('Q');
  lock_task_imu_zero();
  ...
  feed_wall(1); assert(vision_wall_confirmed);
  clock_ms += 20; Auto_Update();
  assert(auto_state == AUTO_WALL_AVOID && wall_avoid_phase == WALL_AVOID_STOP);
  assert(left_pwm_percent == 0 && right_pwm_percent == 0);
  clock_ms += VISION_WALL_STOP_MS; fresh(); Auto_Update();
  assert(wall_avoid_phase == WALL_AVOID_BACKUP);
  clock_ms += 20; fresh(); Auto_Update();
  assert(left_pwm_percent < 0 && right_pwm_percent < 0);
  clock_ms += VISION_WALL_BACKUP_MS; fresh(); Auto_Update();
  assert(wall_avoid_phase == WALL_AVOID_TURN);
  clock_ms += 20; fresh(); Auto_Update();
  assert((left_pwm_percent < 0 && right_pwm_percent > 0) ||
         (left_pwm_percent > 0 && right_pwm_percent < 0));
  feed_wall(0); feed_wall(0); assert(!vision_wall_confirmed);
  clock_ms += VISION_WALL_TURN_MIN_MS; fresh(); Auto_Update();
  assert(auto_state == AUTO_SCAN && left_pwm_percent == 0 && right_pwm_percent == 0);
  assert(strstr(test_log, "VISION WALL CONFIRMED"));
  puts("PASS: OpenMV W packets require confirmation and preempt Q travel with stop/back/centre-turn/rescan");
}
int main(void)
{
